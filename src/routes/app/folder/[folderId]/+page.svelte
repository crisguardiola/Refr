<script lang="ts">
	...
	const rawFlows = $derived(data.flows ?? []);
	...
	type FolderItem = { type: 'screenshot'; data: Screenshot } | { type: 'flow'; data: FolderFlow };
	const folderItems = $derived.by(() => {
		const items: (FolderItem & { createdAt: Date | string })[] = [
			...screenshots.map((s) => ({ type: 'screenshot' as const, data: s, createdAt: s.createdAt })),
			...rawFlows.map((f) => ({ type: 'flow' as const, data: f, createdAt: f.createdAt }))
		];
		items.sort((a, b) => {
			const da = typeof a.createdAt === 'string' ? new Date(a.createdAt).getTime() : (a.createdAt as Date).getTime();
			const db = typeof b.createdAt === 'string' ? new Date(b.createdAt).getTime() : (b.createdAt as Date).getTime();
			return db - da;
		});
		return items;
	});
	const itemsByMonth = $derived(groupScreenshotsByMonth(folderItems));
	const isEmpty = $derived(screenshots.length === 0 && rawFlows.length === 0);
	...
	const flowCtx = getContext<{
		openFlow: (screenshots: Screenshot[], folderId: number | null) => void;
		openFlowForEdit: (flowId: number) => void;
	}>('flowCanvas');
	let menuOpenForId = $state<number | null>(null);
	let menuOpenForFlowId = $state<number | null>(null);
	...
	async function handleDeleteFlow(e: MouseEvent, flowId: number) {
		e.preventDefault();
		e.stopPropagation();
		menuOpenForFlowId = null;
		try {
			const res = await fetch(`/app/flow/${flowId}/delete`, { method: 'DELETE' });
			const json = await res.json();
			if (json?.success) {
				await invalidateAll();
			}
...
<div class="flex flex-1 flex-col gap-6">
				...
				{#if selectedIds.size > 0}
					<div class="flex items-center gap-2 flex-wrap">
						...
						{#if selectedIds.size > 1}
							<Button variant="outline" size="sm" class="gap-1.5" onclick={() => flowCtx?.openFlow(selectedScreenshots, data.folder?.id ?? null)}>
	...
	{:else}
		<div
			...
			ondrop={handleDrop}
		>
			{#each itemsByMonth as { month, screenshots: monthItems }}
				<section class="space-y-4">
					...
					>
					{#each monthItems as item (item.type === 'screenshot' ? `s-${item.data.id}` : `f-${item.data.id}`)}
				...
				>
					...
					>
						...
						{:else}
							{@const flowItem = item.data}
				<div
					role="listitem"
					class="group relative mb-4 inline-block w-full break-inside-avoid overflow-hidden rounded-lg border border-border bg-muted cursor-pointer"
				>
					<span
						class="absolute left-2 top-2 z-10 inline-flex items-center gap-1 rounded-md bg-primary px-2 py-1 text-xs font-medium text-primary-foreground"
						aria-label="Flow"
					>
					...
					<Popover.Root
						open={menuOpenForFlowId === flowItem.id}
						onOpenChange={(o) => (menuOpenForFlowId = o ? flowItem.id : null)}
					>
						<Popover.Trigger
							class="absolute right-2 top-2 z-10 size-8 rounded-md bg-black/50 text-white opacity-0 transition-opacity group-hover:opacity-100 hover:bg-black/70 flex items-center justify-center"
							...
							<Popover.Content align="end" side="bottom" class="w-44 p-1" onclick={(e) => e.stopPropagation()}>
								<button
									...
									onclick={(e) => {
										e.stopPropagation();
										menuOpenForFlowId = null;
										flowCtx?.openFlowForEdit?.(flowItem.id);
									}}
								>
									<Pencil class="size-4" />
									Edit
								...
								<button
									type="button"
									class="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-sm text-destructive hover:bg-destructive/10 hover:text-destructive"
									onclick={(e) => handleDeleteFlow(e, flowItem.id)}
								>
									<Trash2 class="size-4" />
									Delete
								</button>
					...
					<button
						type="button"
						class="block w-full aspect-square overflow-hidden text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-lg"
						onclick={() => flowCtx?.openFlowForEdit?.(flowItem.id)}
						aria-label="Open flow {flowItem.name ?? flowItem.id}"
					>
						{#if flowItem.thumbnailUrl}
							<img
								src={cloudinaryUrl(flowItem.thumbnailUrl, 'thumbnail')}
								alt={flowItem.thumbnailFileName ?? 'Flow thumbnail'}
								width={400}
								height={400}
								loading="lazy"
							...
							/>
						{:else}
							<div class="flex size-full items-center justify-center bg-muted/50 text-muted-foreground">
								<Workflow class="size-12" />
							</div>
						{/if}
					</button>
				</div>
